...
func NewPortScanner(config *Config) *PortClient {
	...
	// fmt.Println("test:",client.pool.Interval)
	...
	client.pool.Function = func(in interface{}) {
		...
		status, response := nmap.ScanTimeout(value.addr.String(), value.num, 200*config.Timeout)
		// if response != nil {
		// 	fmt.Println("test port:",value.addr.String(), value.num, status, response)
		// }
